...
//vicious mockery feature name
let featName = "Vicious Mockery";
//name of the table that mockeries will be drawn, leave blank if you don't
...
if (actorData.length == 1) { //if only one is selected
    ...
    if (featData !== null) { //if have the required feature
        if (game.user.targets.size == 1) { //if have one target
            ...
            if (featTableName !== "")
                content += `<p>Leaving in blank will roll from table '${featTableName}'.</p>`;
